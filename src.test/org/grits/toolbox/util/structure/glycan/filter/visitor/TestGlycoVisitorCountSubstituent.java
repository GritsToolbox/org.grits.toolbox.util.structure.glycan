...
public class TestGlycoVisitorCountSubstituent
{
    ...
    {
        TestGlycoVisitorCountSubstituent.testXML();
        // test for terminal ms
        TestGlycoVisitorCountSubstituent.testAcid();
    }

    private static void testXML() throws JAXBException
    {
        GlycanFilterSubstituent t_filter = new GlycanFilterSubstituent();
        t_filter.setName("Name");
        t_filter.setLabel("Label");
        t_filter.setDescription("Description");
        t_filter.setMin(0);
        t_filter.setMax(5);
        t_filter.setSubstituent(SubstituentType.SULFATE.getName());
    ...
    private static void testAcid()
    {
        ...
        {
            // build the sugar
            Sugar t_sugar = TestGlycoVisitorCountSubstituent.getSugar();
            // build the filter
            GlycanFilterSubstituent t_filter = new GlycanFilterSubstituent();
            t_filter.setSubstituent(SubstituentType.N_GLYCOLYL.getName());
            // run test
            System.out.println("Search for NGc:"
                    + GlycanFilterOperator.applySubstituentFilter(t_sugar, t_filter));
        }
        catch (Exception e)
        {
            e.printStackTrace();
